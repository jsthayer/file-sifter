...
<dd>Filter to screen files before they are loaded into the index. Multiple filters may be specified. (When scanning file systems, even if a directory is rejected by this filter, its contents are still scanned. See <strong>--prunefilter</strong> for controlling recursive scanning.)
</dd>
<dt><strong>-P</strong>, <strong>--prunefilter=FILTER-EXP</strong></dt>
<dd>Filter that is only applied to <em>directories</em>, and then only while scanning actual file systems (not while loading FSIFT files). Any directory that fails to match this filter will be ignored, and the scan will not descend into that directory.
</dd>
