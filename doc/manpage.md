...
fsift - File Sifter: Powerful tool to scan, compute digests and compare files.
Files can be scanned in directory trees or loaded from previously saved results.
...
File Sifter is a utility that has features inspired by the \*NIX utilities
...
files, such as determining which files may be out-of-date in off-site backups.

# OVERVIEW

File Sifter able to scan file systems like *find*, but instead of immediately
...
comparisons. Multiple roots may be specified for each side.
...
Each class of information about a file processed by the program is called a
...
analysis, such as whether the file *matches* at least one other file belonging
to the opposite side of the index.
...
* Compare the contents of a directory tree to a previously saved *FSIFT* file:

>   **fsift t.FSIFT : t/ --diff** --md5 --sort path
...
* Compare the contents of a directory tree to a previously saved *FSIFT* file, showing just differences:

>   **fsift t.FSIFT : t/ --diff**
...
files. Before decommissioning a disk, scan it to check against the archives to
find any files that may need to be added to archives:

>   **fsift archive-index/\*.FSIFT : path/to/disk --membership R --key base,md5**

>>   *or*

>   **fsift archive-index/\*.FSIFT : path/to/disk -mR -kb5**
...
**-s**, **--sort=COLUMNS**
 ~ Sort output using these fields, in order of precedence.
   By default, the output is not sorted. For this option, any column name
   may be precede by a "**/**" character to sort in inverse order.
...
**-L**, **--follow-links**
 ~ Follow symbolic links while scanning the file system. By default, when a
   ...
   descend into that directory when this option is in effect.
...
**-f**, **--postfilter=FILTER-EXP**
 ~ After analysis, any entries rejected by this filter are not output. Multiple filters
   may be specified.
...
**-J**, **--json-out**
 ~ Output file entries in JSON format. The output will be an array of JSON
   objects, each with a key-value pair for each column defined in the
...
**o    modestr**
 ~ Mode and permission bits as a human readable string, in
   \*NIX-like format. See the documentation of the Go
...
glob implementations, but there are some differences. See the Go language *regexp*
...
The output of File Sifter may be saved to a file (an extension of **.FSIFT** is
...
In *entry* lines, fields are separated by space characters.  Certain
characters within a field are escaped with backslashes: spaces, newlines,
carriage returns and backslashes.  In addition, there are two special escape
...
names. (However, even in the last column, any spaces at the beginning
...
indexed under the directory. These cumulative sizes are not included
...
The *Indexed* line shows all of the files that pass the *prefilter* stage and
get loaded into the index.

The *Unmatched* line shows all files that did not have a match on the other
side, and the *Matching* line shows the files that did have a match. The
previous two lines are only output if there were roots on both sides.  The
*Output* line shows all of the files that passed the *postfilter* stage and
were printed to the output (or if **--summary** is specified, would have
been output).
...
wrote over the years to help keep track of files from various computer systems.

The first utilities were simple Perl scripts that did a simple scan/sort/diff
on directories. Eventually, I wrote an program in C++ that used SQLite for an
internal engine that had features somewhat similar to this implementation.
However, it was hard to use the SQL-oriented features of that version, and
although I found it very useful and used it for many years, I was never very
happy with it.
